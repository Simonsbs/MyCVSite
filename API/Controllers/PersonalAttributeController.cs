...
namespace MyCVSite.API.Controllers {
	...
	public class PersonalAttributeController : Controller {
		private readonly IPersonalAttributeRepository _repo;

		public PersonalAttributeController(IPersonalAttributeRepository _repo) {
		...
		[HttpPost]
		public IActionResult Create(PersonalAttribute item) {
			if (item == null) {
				return BadRequest();
			}
		...
		[HttpPut]
		public IActionResult Update(PersonalAttribute item) {
			if (item == null) {
				return BadRequest();
			}

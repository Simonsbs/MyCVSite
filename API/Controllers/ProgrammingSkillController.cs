...
namespace MyCVSite.API.Controllers {
	...
	public class ProgrammingSkillController : Controller {
		private readonly IProgrammingSkillRepository _repo;

		public ProgrammingSkillController(IProgrammingSkillRepository _repo) {
		...
		[HttpPost]
		public IActionResult Create(ProgrammingSkill item) {
			if (item == null) {
				return BadRequest();
			}
		...
		[HttpPut]
		public IActionResult Update(ProgrammingSkill item) {
			if (item == null) {
				return BadRequest();
			}
